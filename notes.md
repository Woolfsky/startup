...
Here are some Cookie attributes that help with security:
+ **Domain**: Specifies the domain for which the cookie is valid. The browser will only send the cookie back to the server if the current domain matches the domain specified in the cookie.
+ **Path**: Specifies the URL path for which the cookie is valid. The cookie will only be sent to the server if the path in the request URL matches the path specified in the cookie.
+ **SameSite**: Determines when a cookie should be sent with cross-site requests (e.g., requests initiated by third-party websites). It helps mitigate the risk of cross-site request forgery (CSRF) and cross-site scripting (XSS) attacks.
+ **HTTPOnly**: When set, this attribute prevents the cookie from being accessed through JavaScript. It is a security measure to mitigate the risk of cross-site scripting (XSS) attacks.
...
### NPM
What role does npm play in web development? The Node Package Manager (NPM) is essential for leveraging preexisting JavaScript packages in your projects. To use a package with Node.js, you first install it locally using NPM, then include a require statement in your code. NPM, automatically installed with Node.js, provides access to a vast repository of packages. Before installing packages, initialize your code by creating a directory and running npm init. You can use npm init -y to quickly accept default settings without a Q&A session.

What does package.json do in a npm project? When you initialize a project with npm, it creates a package.json file containing metadata, scripts, and dependencies. The file includes default metadata and a placeholder script. You can add/remove dependencies, update metadata, and define scripts. Installing a package, like "give-me-a-joke," is done with npm install. The updated package.json includes a description, a useful script ("dev": "node index.js"), and the installed package as a dependency. npm creates package-lock.json and node_modules to track versions and manage dependencies. Always run npm install when cloning a project to ensure dependencies are installed.

### Locally testing web server without AWS instance running
Though I've shut down my AWS server, I can still run the project locally and interact with it. Open the correct directory `cd ...Mokykla/Fall-2023/CS260/startup-REACT/` then run `npm run dev` to run the front-end code (type `o` in the terminal then the `enter` button to open the code in the browser). Now that this is running, start up the back-end service with Mongo DB. Open up another terminal window and again navigate to that directory and then enter the `service` directory: `cd ...Mokykla/Fall-2023/CS260/startup-REACT/service/`. Once here, run `node index.js`. Your front-end and back-end are now running!
